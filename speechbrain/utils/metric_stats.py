...
# class MultiBinaryMetricStats:
#     def __init__(self, keys):
#         self.metric_stats = {}
#         for k in keys:
#             self.metric_stats[k] = BinaryMetricStats(positive_label=k)

#     def clear(self):
#         for _, metric in self.metric_stats.items():
#             metric.clear()
...
#     def summarize(self):
#         summaries = {}
#         for key, metric in self.metric_stats.items():
#             summaries[key] = metric.summurize()
#         return summaries


class SklearnMetricStats(MetricStats):
    def __init__(self):
        self.clear()
    ...
    def append(self, ids, scores, labels):
        ...
        Does not compute metrics until time of summary.
        ...
        """
        self.ids.extend(ids)
        self.scores.extend(scores.argmax(dim=1).detach().cpu().numpy())
    ...
    def summarize(self, field=None):
        ...
        self.summary["F1"] = f1_score(
            self.labels,
            self.scores,
            average=None,
        ).tolist()

        m_p, m_r, m_fs, _ = precision_recall_fscore_support(
            self.labels,
            self.scores,
            average="micro",
        )

        self.summary["micro"] = (m_p.item(), m_r.item(), m_fs.item())
        ...
        self.summary["macro"] = (m_p.item(), m_r.item(), m_fs.item())
    ...
    def write_stats(self, filestream, verbose=False):
        ...
        message = f"F1 score: : {self.summary['F1']}\n"
        message += f"Micro: precision: {self.summary['micro'][0]}, "
        message += f"recall: {self.summary['micro'][1]}, "
        message += f"fscore: {self.summary['micro'][2]}\n"
        message += f"Macro: precision: {self.summary['macro'][0]}, "
        message += f"recall: {self.summary['macro'][1]},"
        message += f"fscore: {self.summary['macro'][2]}\n"
